...
import { ArrowLeft, Phone, Languages, Sparkle, ChevronDown } from "./icons";
...
export default function ChatView({ contact, onBack, onAttach, settings }) {
    ...
    const unsub = subscribe(contact.user_id, (incoming) => {
      if (Array.isArray(incoming)) {
        ...
        setMessages((prev) => {
          ...
          setNewMsgIds((s) => new Set([...s, incoming.message_id]));
          if (incoming.sender_id !== "u_me") setUnreadCount((c) => c + 1);
          setTimeout(() => {
            setNewMsgIds((s) => { const n = new Set(s); n.delete(incoming.message_id); return n; });
          }, 3000);
  ...
  const handleScroll = useCallback(() => {
    ...
    if (!el) return;
    if (el.scrollHeight - el.scrollTop - el.clientHeight < 80) setUnreadCount(0);
  }, []);
  ...
  function handleSend({ text, sourceLang }) {
    ...
    if (isDemo) {
      setTimeout(() => simulateReply(contact.user_id, myLang), 1400 + Math.random() * 1600);
    }
  }
    ...
    <div className="flex h-full min-h-0 flex-col">
      <header className="z-30 flex items-center gap-3 border-b border-line bg-surface/80 px-3 py-2.5 backdrop-blur-xl sm:px-4">
        ...
        <div className="flex items-center">
          <button className="btn-ghost h-9 w-9" aria-label="Call">
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="chat-canvas min-h-0 flex-1 overflow-y-auto scrollbar-thin relative">
        <div className="mx-auto flex max-w-3xl flex-col gap-3 px-3 py-6 sm:px-4">
          <IntroCard contact={contact} myLang={myLang} />
          {groups.map((g) => (
...
function IntroCard({ contact, myLang }) {
  const contactLang = LANGUAGES[contact.preferred_language];
  const myLanguage = LANGUAGES[myLang];
  return (
    <div className="mx-auto mb-2 flex max-w-md flex-col items-center gap-3 rounded-2xl bg-surface/70 px-6 py-6 text-center ring-1 ring-line backdrop-blur-sm">
      ...
      <p className="text-[13px] leading-relaxed text-muted pretty max-w-[35ch]">
        <span className={cx("font-medium text-ink", myLanguage?.font)}>{myLanguage?.native}</span>
        {" → "}
        <span className={cx("font-medium text-ink", contactLang?.font)}>{contactLang?.native}</span>
        {" · "}
